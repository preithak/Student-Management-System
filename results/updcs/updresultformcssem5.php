...
if(isset($_POST['but1'])){
	...
	$sub5=$_POST['sub5'];
	$sub6=$_POST['sub6'];
	$que="select * from cssem5";
	$run=mysqli_query($db,$que);
	while($row=mysqli_fetch_array($run)){
		$user=$row['ID'];
	}
		if($id==$user){
			$update="update cssem5 set Subject1='$sub1', Subject2='$sub2', Subject3='$sub3', Subject4='$sub4',
				Subject5='$sub5', Subject6='$sub6' where ID='$id' ";
				$run1=mysqli_query($db,$update);
		}//if for update
		else{
			$insert="insert into cssem5 values ('$id', '$sub1', '$sub2', '$sub3', '$sub4', '$sub5', '$sub6')";
